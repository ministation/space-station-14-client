        ...
        out string error)
    {
        ...
        try
        {
            ...
            lock (this)
            {
                ...
                Vector2 sum = default;
                var nSum = 0;
                // Cull around the rendered camera rather than the unpanned eye.
                // Far state remains cached until LeavePvs.
                var focus = worldEye + cameraOffset;
                const float viewTiles = 52f;
                var viewR2 = viewTiles * viewTiles;
                var smoothR2 = viewR2 * 1.35f;

                // IconSmooth occupancy: GRID-LOCAL tile + parent (PC Snapgrid).
                // Only near the eye — full-store scan was a major Apply() cost.
                ...
                foreach (var (ent, xf0) in _xforms)
                {
                    ...
                    var dx0 = wp0.X - focus.X;
                    var dy0 = wp0.Y - focus.Y;
                    ...
                    if (string.IsNullOrEmpty(path0))
                        continue;
                    if (!TryResolveIconSmooth(proto0, path0, out var smooth0) || smooth0.Mode == IconSmoothMode.NoSprite)
                        continue;
            ...
            foreach (var (chunkIndex, datum) in chunks)
            {
                ...
                for (var i = 0; i < tiles.Length; i++)
                {
                    ...
                    list.Add(new WorldTileDraw(
                        ...
                        tile.Variant, tile.RotationMirroring));
                }
            }
        }
